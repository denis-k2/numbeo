...
async def read_city_list(
    ...
    if country_code:
        ...
        db_city_by_country = await crud.get_city_list(db)
...
async def read_city(
    ...
    if numbeo_indices:
        db_numbeo_indices = await crud.get_numbeo_city_indices(db, city_id)
        result["numbeo_indices"] = jsonable_encoder(db_numbeo_indices)
    if avg_climate:
        db_avg_climate = await crud.get_climate(db, city_id)
...
async def read_country(
    ...
    if numbeo_indices:
        db_numbeo_ctry_idx = await crud.get_numbeo_ctry_idx(db, country_code)
        if db_numbeo_ctry_idx is not None:
            result["numbeo_indices"] = jsonable_encoder(db_numbeo_ctry_idx)
    if legatum_indices:
        db_legatum_idx = await crud.get_legatum_idx(db, country_code)
        if db_legatum_idx:
            legatum_dict = {}
